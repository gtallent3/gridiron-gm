...
const TinyBtn = (props: React.ButtonHTMLAttributes<HTMLButtonElement>) => (
  <button
    {...props}
    className={
      'px-2.5 py-1 rounded-lg text-[11px] border border-zinc-700/70 ' +
      'bg-zinc-900/80 hover:bg-zinc-800 transition ' +
      (props.className || '')
    }
  />
)
...
export default function StartSit() {
  ...
  useEffect(() => {
    fetch('/api/players').then(r => r.json()).then(d => setList(d.players))
  }, [])
  ...
  return (
    <Section title="Start / Sit Assistant" right={<Badge tone="emerald">Alpha</Badge>}>
      ...
      <div className="mb-4 grid grid-cols-1 sm:grid-cols-[1fr_auto] gap-2">
        <div className="flex items-center rounded-xl border border-zinc-700/70 bg-black/50 px-3 py-2">
          <input
        ...
        <button
          className="rounded-xl border border-emerald-600/30 bg-emerald-500/15 px-3 py-2 text-sm text-emerald-300 hover:bg-emerald-500/25"
          type="button"
        >
          Must-Starts 🔥
        </button>
        ...
        {filtered.map((p: any) => (
          ...
          >
            <div className="flex items-start justify-between">
              <div>
                <div className="text-zinc-100 font-semibold leading-tight">
                  {p.name}{' '}
                  <span className="text-zinc-400 font-normal">({p.team} · {p.pos})</span>
                </div>
